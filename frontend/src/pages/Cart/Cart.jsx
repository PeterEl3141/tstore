  ...
  return (
    <section className="cart">
      ...
      <ul className="cart-list">
        {items.map((i) => (
          <li key={`${i.id}|${i.size ?? ""}|${i.color ?? ""}`} className="cart-item">
            {i.image && <img className="cart-item-image" src={i.image} alt={i.name} />}
            <div className="cart-item-meta">
              ...
              <div className="cart-item-price">{(i.priceCents / 100).toFixed(2)} {i.currency}</div>
              <div className="cart-item-actions">
                <input
                  type="number"
                  min="1"
                  value={i.qty}
                  onChange={(e) => setQty(i.id, i.size, i.color, Math.max(1, Number(e.target.value) || 1))}
                />
                <button onClick={() => remove(i.id, i.size, i.color)}>Remove</button>
              </div>
            </div>
          </li>
        ))}
